...
for patch_name in patch_names:
    ...
    speedup_log = []
    for c, unf_t, filt_t in zip(c_range, times_by_variant["naive"], times_by_variant["smart"]):
        speedup = unf_t / filt_t if filt_t > 0 else float("inf")
        speedup_log.append(f"c={c}: naive={unf_t:.3f}s, smart={filt_t:.3f}s, speedup={speedup:.2f}x")
    ...
    ax.set_title(f"{patch_name}: runtime vs c (smart vs naive)")
    ax.legend()
